export default function PartnersAndSubsidiaries() {
  const partners = [
    {
      name: "NELLYVAL GROUP",
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {partners.map((partner) => (
              <a
                key={partner.name}
                href={partner.link}
                className="block rounded-xl border border-slate-200 bg-slate-50/60 p-5 shadow-sm group hover:shadow-xl transition-all duration-300 no-underline focus:outline-none focus:ring-4 focus:ring-blue-500/20"
              >
                {/* Image for partners */}
                ...
                <div className="mt-4 pt-4 border-t border-slate-200">
                  ...
                  <p className="text-sm font-medium text-indigo-600 hover:text-indigo-700 transition-colors mb-2">
                    📧 {partner.email}
                  </p>
                  <p className="text-sm font-medium text-indigo-600 hover:text-indigo-700 transition-colors mb-2">
                    📞 {partner.phone}
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {subsidiaries.map((sub) => (
              ...
              >
                ...
                <div className="mt-4 pt-4 border-t border-slate-200">
                  <p className="text-xs text-slate-500 mb-3">Contact:</p>
                  <p className="text-sm font-medium text-emerald-600 hover:text-emerald-700 transition-colors mb-2">
                    📧 {sub.email}
                  </p>
                  <p className="text-sm font-medium text-emerald-600 hover:text-emerald-700 transition-colors mb-2">
                    📞 {sub.phone}
